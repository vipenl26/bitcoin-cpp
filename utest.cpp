...
BOOST_AUTO_TEST_CASE(generate_public_key) {
    ...
    uint256_t expected_x("83998262154709529558614902604110599582969848537757180"
                         "553516367057821848015989"),
        expected_y("37676469766173670826348691885774454391218658108212372128812"
                   "329274086400588247");
...
BOOST_AUTO_TEST_CASE(sha256_test) {
    ...
    BOOST_CHECK(
        compute_sha256("here is a random bytes message, cool right?") ==
...
BOOST_AUTO_TEST_CASE(ripemd160_test) {
    ...
    // empty string to hash
    BOOST_CHECK(compute_ripemd160("") ==
                "9c1185a5c5e9fc54612808977ee8f548b2258d31");

    BOOST_CHECK(compute_ripemd160("hello this is a test") ==
...
BOOST_AUTO_TEST_CASE(ripemd160_sha256_test) {
    ...
    BOOST_CHECK(
        uint8_to_hex_string(compute_sha256(string_to_bytes(s))) ==
        "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
    BOOST_CHECK(bytes_to_hex(compute_ripemd160(string_to_bytes(s))) ==
                "9c1185a5c5e9fc54612808977ee8f548b2258d31");

    BOOST_CHECK(uint8_to_hex_string(
                    compute_ripemd160(compute_sha256(string_to_bytes(s)))) ==
    ...
    BOOST_CHECK(uint8_to_hex_string(
                    compute_ripemd160(compute_sha256(string_to_bytes(s)))) ==
                "71e8a0cf93adcc523a456a908c948aa842f343f8");
}
